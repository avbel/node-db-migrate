...
module.exports = Base = Class.extend({
  ...
  mapDataType: function(spec) {
    switch(spec.type) {
      ...
      case type.BINARY:
        return 'BINARY';
      case type.NUMERIC:
        return 'NUMERIC(' + (spec.digits || 20) + ',' + (spec.decimals || 2) + ')';
      case type.BOOLEAN:
        return 'BOOLEAN';
      default:
  ...
  createColumnDef: function(name, spec, options) {
    var type       = this.mapDataType(spec);
    var len        = spec.length ? util.format('(%s)', spec.length) : '';
    if(spec.type == type.NUMERIC){
      var digits = spec.digits || 10;
      var decimals = spec.decimals || 0;
      len = util.format('(%d, %d)', digits, decimals);
    }
  ...
  addIndex: function(tableName, indexName, columns, opts, callback) {
    ...
    var sql = util.format('CREATE %s INDEX %s ON %s (%s)', uniqueSql, indexName, tableName, columns.join(', '));
