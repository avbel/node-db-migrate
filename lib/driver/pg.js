...
var PgDriver = Base.extend({
    ...
    for(i = 0; i < l; i++){
      parentCols.push("'" + parentColumns[i] + "'");
    }
    ...
    var sql = util.format("ALTER TABLE %s ADD CONSTRAINT %s FOREIGN KEY (%s) REFERENCES %s(%s)%s%s",  tableName, foreignKeyName, cols.join(', '), parentTableName, parentCols.join(', '), onUpdateAction, onDeleteAction);
    callback = callback || opts;
    this.runSql(sql, callback);
  },

  removeForeignKey: function(tableName, foreignKeyName, callback) {
    var sql = util.format("ALTER TABLE '%s' DROP CONSTRAINT KEY %s",  tableName, foreignKeyName);
    this.runSql(sql, callback);
  },
